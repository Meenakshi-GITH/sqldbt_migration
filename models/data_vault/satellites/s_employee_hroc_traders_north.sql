...
SELECT DISTINCT
    ...
    stage_employee_traders_north.extension AS extension,

    stage_employee_traders_north.dss_change_hash_employee_hroc_traders_north
        AS dss_change_hash,
...
WHERE NOT EXISTS (
    ...
    FROM {{ this }} AS s_employee_hroc_traders_north

    WHERE stage_employee_traders_north.hk_h_employee =
          s_employee_hroc_traders_north.hk_h_employee

      AND stage_employee_traders_north.dss_change_hash_employee_hroc_traders_north =
          s_employee_hroc_traders_north.dss_change_hash

      AND current_rows.dss_start_date =
          s_employee_hroc_traders_north.dss_start_date
