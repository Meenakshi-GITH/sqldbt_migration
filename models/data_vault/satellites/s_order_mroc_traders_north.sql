...
with stage_order as (
    ...
    select distinct
        ...
        shippostalcode,
        shipcountry,
        dss_change_hash_order_mroc_traders_north,
        dss_record_source,
        dss_load_date
    from {{ ref('stg_order_traders_north') }}
...
select distinct
    ...
    stage_order.requireddate as requireddate,
    stage_order.shippeddate as shippeddate,
    stage_order.shipname as shipname,
    stage_order.shipaddress as shipaddress,
    stage_order.shipcity as shipcity,
    stage_order.shipregion as shipregion,
    stage_order.shippostalcode as shippostalcode,
    stage_order.shipcountry as shipcountry,
    stage_order.dss_change_hash_order_mroc_traders_north as dss_change_hash,
...
where not exists (
    ...
    where stage_order.hk_h_order = target.hk_h_order
      and stage_order.dss_change_hash_order_mroc_traders_north =
