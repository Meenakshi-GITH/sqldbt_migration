...
SELECT DISTINCT
    ...
    stage_product_traders_south.productid AS productid,

    stage_product_traders_south.reorderlevel AS reorderlevel,

    stage_product_traders_south.discontinued AS discontinued,

    stage_product_traders_south.dss_change_hash_product_lroc_traders_south
        AS dss_change_hash,
...
WHERE NOT EXISTS (
    ...
    FROM {{ this }} AS s_product_lroc_traders_south

    WHERE h_product.hk_h_product =
          s_product_lroc_traders_south.hk_h_product

      AND stage_product_traders_south.dss_change_hash_product_lroc_traders_south =
          s_product_lroc_traders_south.dss_change_hash

      AND current_rows.dss_start_date =
          s_product_lroc_traders_south.dss_start_date
